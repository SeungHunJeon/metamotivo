...
# TRACKING METRICS
#####################
def distance_proximity(next_obs: torch.Tensor, tracking_target: torch.Tensor, mask: torch.Tensor, bound: float = 2.0, margin: float = 2):
    stats = {}
    ...
    # Iterate over the number of environments (num_env)
    for env_idx in range(next_obs.shape[1]):  # next_obs.shape[1] is num_env
        # Apply mask to next_obs and tracking_target for the current environment
        next_obs_env = next_obs[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]
        tracking_target_env = tracking_target[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]

        # Calculate distance using distance_smpl function
        dist = distance_smpl(next_obs_env, tracking_target_env)

        # Calculate proximity and success for the current environment
        in_bounds_mask = dist <= bound
        out_bounds_mask = dist > bound + margin

        proximity = (in_bounds_mask + ((bound + margin - dist) / margin) * (~in_bounds_mask) * (~out_bounds_mask)).mean()
        proximity_values.append(proximity.item())

        distance_values.append(dist.mean().item())

        # Success: True if the distance is within bounds for any timestep in the environment
        success_values.append(in_bounds_mask.min().float().item())

    # Aggregate stats (mean over environments)
    stats["proximity"] = proximity_values
    stats["distance"] = distance_values
    stats["success"] = success_values
...
def emd_numpy(next_obs: torch.Tensor, tracking_target: torch.Tensor, mask: torch.Tensor):
    # keep only pose part of the observations
    agent_obs = get_pose(next_obs).to("cpu")
    tracked_obs = get_pose(tracking_target).to("cpu")
    # compute optimal transport cost
    # Initialize a list to store EMD for each environment
    emd_values = []

    # Iterate over the number of environments (num_env)
    for env_idx in range(agent_obs.shape[1]):  # agent_obs.shape[1] is num_env
        # Extract observations for the current environment
        agent_env_obs = agent_obs[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]
        tracked_env_obs = tracked_obs[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]

        # Compute the cost matrix for the current environment
        cost_matrix = distance_matrix(agent_env_obs, tracked_env_obs).cpu().detach().numpy()

        # Define the potentials (uniform distribution)
        X_pot = np.ones(agent_env_obs.shape[0]) / agent_env_obs.shape[0]
        Y_pot = np.ones(tracked_env_obs.shape[0]) / tracked_env_obs.shape[0]

        # Compute the optimal transport cost (EMD) for the current environment
        transport_cost = ot.emd2(X_pot, Y_pot, cost_matrix, numItermax=100000)

        # Append the EMD for this environment to the list
        emd_values.append(transport_cost)

    # Return a dictionary with the EMD values for each environment
    return {"emd": emd_values}

ROOT_H_OBS = 1
LOCAL_BODY_POS = 69


def phc_metrics(next_obs: torch.Tensor, tracking_target: torch.Tensor, mask: torch.Tensor):
    """
    Calculation of metrics used in the PHC paper, adapted for environments with masking.
    """
    stats = {}

    # Determine the indices for the root and local body positions
    xpos_idxs = [ROOT_H_OBS, ROOT_H_OBS + LOCAL_BODY_POS]

    # Initialize a list to store results for each environment
    mpjpe_g_values = []
    vel_dist_values = []
    accel_dist_values = []
    success_phc_linf_values = []
    success_phc_mean_values = []

    # Iterate over the number of environments (num_env)
    for env_idx in range(next_obs.shape[1]):  # next_obs.shape[1] is num_env
        # Apply the mask to the environment observations
        next_obs_env = next_obs[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]
        tracking_target_env = tracking_target[:, env_idx, :][~mask[:, env_idx].squeeze(-1)]

        # Extract the joint positions for the current environment
        jpos_pred = next_obs_env[..., xpos_idxs[0] : xpos_idxs[1]]  # (seq_len, 69)
        jpos_gt = tracking_target_env[..., xpos_idxs[0] : xpos_idxs[1]]  # (seq_len, 69)

        # MPJPE (Mean Per Joint Position Error) in global space
        mpjpe_g = torch.norm(jpos_gt - jpos_pred, dim=1).mean() * 1000
        mpjpe_g_values.append(mpjpe_g.item())

        # Compute the velocity as finite difference
        vel_gt = jpos_gt[1:, :] - jpos_gt[:-1, :]
        vel_pred = jpos_pred[1:, :] - jpos_pred[:-1, :]
        vel_dist = torch.norm(vel_pred - vel_gt, dim=1).mean() * 1000
        vel_dist_values.append(vel_dist.item())
        ...
        accel_dist = torch.norm(accel_pred - accel_gt, dim=1).mean() * 1000
        accel_dist_values.append(accel_dist.item())

        # Success metric for PHC
        ...
        success_phc_linf = torch.all(torch.norm(jpos_pred - jpos_gt, dim=-1) <= 0.5).float()
        success_phc_mean = torch.all(torch.norm(jpos_pred - jpos_gt, dim=-1).mean(dim=-1) <= 0.5).float()

        success_phc_linf_values.append(success_phc_linf.item())
        success_phc_mean_values.append(success_phc_mean.item())

    # Aggregate stats (mean over environments)
    stats["mpjpe_g"] = mpjpe_g_values
    stats["vel_dist"] = vel_dist_values
    stats["accel_dist"] = accel_dist_values
    stats["success_phc_linf"] = success_phc_linf_values
    stats["success_phc_mean"] = success_phc_mean_values
